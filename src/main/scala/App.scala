...
object App extends FinatraServer {
  ...
  class ExampleApp extends Controller {
    ...
    get("/") { request =>
      respondTo(request) {
        case r: Json => {
          val boundaryObjOpt = request.params.get("boundary")
          val pointObjOpt = request.params.get("point")

          val isWithInBoundary: Option[Boolean] = for {
            boundaryStr <- boundaryObjOpt
            pointStr    <- pointObjOpt
